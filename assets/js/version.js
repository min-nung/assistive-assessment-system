...
const VERSION_HISTORY = Object.freeze([
  Object.freeze({
    ...
    updates: Object.freeze([
      '刪除個案不再是永久的。資料會移到「回收筒」保留 7 天，期間隨時可以還原，超過期限才自動清除。',
      '回收筒會跟著備份一起上傳雲端，所以即使刪除已經同步出去，資料仍在那份快照裡——換一台裝置也還原得回來。',
      '刪除後立即跳出「復原」提示約 8 秒，手滑的時候按一下就救回來，不必進設定。',
      '設定選單新增「回收筒」，可查看每筆還剩幾天、還原，或提前永久刪除。',
      '還原時若已有同名個案會擋下並提示，不會產生兩筆分不出來的個案。',
      '匯出的 JSON 備份也會包含回收筒內容；1.7 以前的舊備份檔仍可正常匯入。'
    ])
  }),
  Object.freeze({
    version: '1.7',
    ...
    title: '修正自動備份沒有自動執行',
    updates: Object.freeze([
      '修正重新載入頁面後自動備份不會執行的問題。原本必須先開啟「設定」或「雲端備份」才會取得授權，備份才會恢復——這正是「按了才會更新備份」的原因。',
      '現在只要開啟後碰一下畫面（點個案、點輸入框、手指按下捲動都算），就會在背景無聲取得授權，不必碰任何跟備份有關的按鈕，也不會跳出任何提示。',
      '授權快到期時會提前更新，因此連續使用超過一小時也不會中斷備份。',
      '約每週一次的授權到期會跳出視窗提醒重新連結，按一下即可恢復；選擇「稍後再說」則同一次使用不會再打擾，標題列仍會持續顯示備份已停止。'
    ])
  }),
  Object.freeze({
    version: '1.6',
